 ...
 * @LastEditors: shaoyun
 * @LastEditTime: 2019-09-09 21:24:02
 * @Description: 牛客网面试题总结(美团、小米)
 -->
## 面试题总结
  ...
  - 错误捕获：由于promise中发生的未捕获异常不会被window.onerror捕获，会导致无法自动收集未捕获的promise异常，因此需要进行异常捕获。
    - 在promise中直接catch：
      ...
      p1.then(function(value) {
        ...
        console.log('catched:', error) // Boom!
      })
      ```
...
11. v-cloak属性：从后台读取数据或是竖向页面时由于响应问题，会闪现{{item.name}}模板变量，这个属性用来防止vue加载时出现变量名。
