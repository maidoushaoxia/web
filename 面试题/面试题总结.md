 ...
 * @LastEditors: shaoyun
 * @LastEditTime: 2019-09-03 21:12:47
...
6. hash和history模式
  - hash路由：URL里有一个#，这有这个符号之前的内容会被包含在请求中

  - history模式：前端的URL必须和实际发起请求的URL一致

## Webpack部分
1. loader和plugins的区别：
  - loader：webpack将所有文件都视为模块，但只能解析js文件，loader是用来加载和解析非js文件的。如CSS-Loader、Style-Loader、babel-loader、eslint-loader等。

  - plugins：是一个具有apply属性的js对象，解决loader无法处理的问题。如html-webpack-plugin（简化html文件）等。webpack会在特定的时间点广播出特定的事件，插件在监听到感兴趣的事件后执行特定的逻辑，且插件可以调用webpack提供的API改变webpack的运行结果。
